...
const AddUpcomingMeal = ({refetch}) => {
        ...
        fetch("http://localhost:5000/upcoming-meals", {
            ...
            .then(data => {
                if (data.insertedId) {
                    Swal.fire({
                        title: "Added!",
                        text: "A new meal added in upcoming list successfully.",
                        icon: "success"
                    });
                    refetch();
                }
    ...
    return (
        <div className="card bg-base-100 w-full max-w-4xl mx-auto">
            <h2 className="text-3xl font-semibold text-center mt-3">Add an Upcoming Meal.</h2>
            <form method="dialog" onSubmit={handleAddMeal} className="card-body">
                {/* Row 01 */}
                ...
                <div className="form-control mt-6">
                    <button className="btn btn-warning">Add Meal</button>
                </div>
            </form>
        </div>
    );
